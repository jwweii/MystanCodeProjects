...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name not in name_data:
        ...
        if year in name_data[name]:
            # if the name is both in boy and girl, the system will save the one with lower rank in that year.
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            tokens = line.split(',')
            if len(tokens) == 1:  # get the year of the file (only one word in the line).
                year = tokens[0].strip()
            else:
                rank = tokens[0].strip()
